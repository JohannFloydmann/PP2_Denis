...
class Snake(pygame.sprite.Sprite):
    def __init__(self, body, score, level, level_progress, level_change, dx, dy):
        super().__init__() 
        self.body = body
        self.dx = int(dx)
        self.dy = int(dy)
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN and self.active_index == 0:
                    # Move to the next scene when the user pressed Enter
                    self.SwitchToScene(GameScene(Snake(body= [Point(10, 11), Point(10, 12), Point(10, 13)],
                                                       score = 0, level = 1,
                                                       level_progress = 1, level_change = 3,
                                                       dx=1, dy=0)))
                elif event.key == pygame.K_RETURN and self.active_index == 1:
                    save = db.get_save(user_name)
                    if save:
                        snake = Snake(body=[Point(*segment) for segment in save["snake"]],
                                      score=save["score"],
                                      level=save["level"],
                                      level_progress=save["level_progress"],
                                      level_change=save["level_change"],
                                      dx=save["dir_x"], dy=save["dir_y"])
                        self.SwitchToScene(GameScene(snake))
                    else:
...
class GameScene(SceneBase):
    ...
    def __init__(self, snake=None):
        ...
        print(f"GameScene initialized. Snake body: {[str(p) for p in self.snake.body]}")
        print(f"Snake score: {self.snake.SCORE}, level: {self.snake.LEVEL}, FPS: {FPS}")
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RIGHT and self.snake.dx != -1:
                    ...
                    save = {
                        ...
                        "level_progress": self.snake.LEVEL_PROGRESS,
                        "level_change": self.snake.LEVEL_CHANGE,
                        "snake": [(segment.x, segment.y) for segment in self.snake.body],
                        "dir_x": self.snake.dx,
                        "dir_y": self.snake.dy
                    }
                    db.load_save(user_name, save)
                    print("Game saved!")
    ...
    def Update(self):
        ...
        if self.snake.check_self_collision():
            ...
            level = self.snake.LEVEL
            self.snake.kill()
            self.SwitchToScene(EndScene(score, level))
